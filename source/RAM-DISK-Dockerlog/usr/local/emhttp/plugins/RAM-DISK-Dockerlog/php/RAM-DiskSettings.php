...
function getCaseContentForVersion($filePath, $version) {
    ...
    return trim($caseContent);
}

$filePath = '/usr/local/emhttp/plugins/RAM-DISK-Dockerlog/event/starting';

$versionToDisplay = trim(shell_exec('source /etc/unraid-version && echo $version'));

$caseContent = getCaseContentForVersion($filePath, $versionToDisplay);
...
<head>
    ...
    <title>RAM-Disk for Docker settings</title>
</head>
<body>
...
<div style="width:99%; float:left; border: 0em solid black;">
    <h2>Contents for Version: <?php echo htmlspecialchars($versionToDisplay); ?></h2>
    <p><?php check_file_exists($modcheck); ?></p>
    <pre><?php echo htmlspecialchars($caseContent ?: 'No script found for this version'); ?></pre>
</div>
</body>
</html>
